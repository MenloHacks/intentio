...
exports.transform = function (session, url, options) {
    ...
    var validate = schema_1.getValidator(schema_1.TransformSchema);
    // use lower case only for validation
    if (!validate(schema_1.valuesToLowerCase(JSON.parse(JSON.stringify(options))))) {
        throw new FilestackError_1.FilestackError('Validation error', validate.errors);
    }
    var transformsArray = [];
